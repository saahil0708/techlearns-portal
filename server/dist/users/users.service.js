...
export const userSanitizedSelect = {
    id: true,
    email: true,
    name: true,
    globalRole: true,
...
let UsersService = UsersService_1 = class UsersService {
    ...
    constructor(prisma, configService, mailService) {
        ...
        const currentKey = this.configService?.get('auth.totp.encryptionKey') || process.env.TOTP_ENCRYPTION_KEY;
        const previousKeys = this.configService?.get('auth.totp.previousEncryptionKeys') ||
        ...
        this.encryptionKeys = rawKeys.map((key) => Buffer.from(createHmac('sha256', key).update('codeplatform:invitation:v1').digest()));
    }
    ...
    async changePassword(userId, currentPass, newPass) {
        ...
        await this.logAdminActivity(userId, 'Password Changed', 'Account security password updated successfully');
        return { success: true, message: 'Password changed successfully' };
    }
    async getStudentProfile(handleOrId) {
        ...
        const mappedCourses = user.enrollments.map((enr) => {
            ...
            const completedLessons = user.lessonProgress.filter((lp) => lp.completed &&
                enr.course.modules.some((m) => m.lessons.some((l) => l.id === lp.lessonId))).length;
            const progressPct = totalLessons > 0 ? Math.min(100, Math.round((completedLessons / totalLessons) * 100)) : 0;
            const modulesCompleted = enr.course.modules.filter((m) => m.lessons.length > 0 && m.lessons.every((l) => completedLessonIds.has(l.id))).length;
            return {
                id: enr.course.id,
        ...
        const mappedContests = user.contestRegistrations.map((cr) => {
            ...
            const totalParticipants = cr.contest._count?.leaderboardEntries ?? 0;
    ...
    async getSuperAdminMetrics(user) {
        const isPlatformAdmin = user?.globalRole === Role.SUPER_ADMIN || user?.globalRole === Role.PLATFORM_ADMIN;
        const institutionIds = user?.memberships?.map((membership) => membership.institutionId || membership.collegeId).filter(Boolean) || [];
        ...
        const [institutionsCount, studentsCount, facultyCount, adminsCount, totalUsersCount, problemsCount, contestsCount, submissionsCount,] = await Promise.all([
    ...
    async getAdminAuditLogs(userId) {
    ...
    async logAdminActivity(userId, action, detail, ipAddress, status = 'SUCCESS') {
        try {
    ...
    async findPaginated(args, role, status, institutionId) {
    ...
    async updateUser(id, input) {
        ...
        if (input.officeHours !== undefined)
            data.officeHours = input.officeHours;
        ...
        if (input.resumeFileName !== undefined)
            data.resumeFileName = input.resumeFileName;
        ...
        if (customHandle !== undefined)
            data.rollNo = customHandle;
        if (input.contestRating !== undefined)
            data.contestRating = input.contestRating;
        if (input.ratingTier !== undefined)
            data.ratingTier = input.ratingTier;
    ...
    async bulkInvite(input) {
        ...
        await this.prisma.$transaction(async (tx) => {
            for (const item of input.users) {
                ...
                if (item.batchId) {
                    ...
                    if (!batch)
                        throw new BadRequestException(`Batch ${item.batchId} does not exist`);
                ...
                if (effectiveInstId) {
                    ...
                    if (!inst)
                        throw new BadRequestException(`Institution ${effectiveInstId} does not exist`);
                }
            }
            for (const item of input.users) {
                const email = item.email.toLowerCase();
        ...
        if (this.mailService) {
            ...
            const batchIds = [...new Set(input.users.map((u) => u.batchId).filter(Boolean))];
            const institutionIds = [...new Set(input.users.map((u) => u.institutionId || u.collegeId).filter(Boolean))];
                        ...
                        if (sendSucceeded) {
                            try {
                                await this.prisma.invitationDelivery.updateMany({
                                    ...
                                    data: { activationUrl: null, status: 'DELIVERED' },
                                });
                            }
                            catch (finalizeErr) {
                                this.logger.error(`Invitation email sent to ${item.email}, but failed to finalize delivery record: ${finalizeErr.message}`);
                            }
                        }
                        else {
                            await this.prisma.invitationDelivery
                                ...
                                .catch((revertErr) => {
                                this.logger.warn(`Failed to reset invitation delivery status to PENDING for ${item.email}: ${revertErr.message}`);
                            });
                        }
                    })
                        .catch((err) => {
    ...
    async processPendingDeliveries(sender) {
        const now = new Date();
        const expiredResult = await this.prisma.invitationDelivery.updateMany({
            ...
            data: { activationUrl: null, status: 'EXPIRED' },
        });
        await this.prisma.invitationDelivery.updateMany({
            where: {
                status: 'CLAIMED',
