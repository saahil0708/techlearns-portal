var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var JudgeProcessor_1;
import { Processor, WorkerHost } from '@nestjs/bullmq';
import { Logger } from '@nestjs/common';
import { EVALUATE_SUBMISSION_JOB, JUDGE_QUEUE_NAME, } from '../judge.constants.js';
import { JudgeService } from '../judge.service.js';
let JudgeProcessor = JudgeProcessor_1 = class JudgeProcessor extends WorkerHost {
    judgeService;
    logger = new Logger(JudgeProcessor_1.name);
    constructor(judgeService) {
        super();
        this.judgeService = judgeService;
    }
    async process(job) {
        this.logger.log(`Processing job ${job.id} (name: ${job.name}) for submission ${job.data.submissionId}`);
        if (job.name === EVALUATE_SUBMISSION_JOB || !job.name || job.name === '__default__') {
            await this.judgeService.evaluateSubmission(job.data.submissionId);
        }
        else {
            this.logger.warn(`Unknown job name: ${job.name}`);
        }
    }
};
JudgeProcessor = JudgeProcessor_1 = __decorate([
    Processor(JUDGE_QUEUE_NAME, {
        concurrency: 5,
    }),
    __metadata("design:paramtypes", [JudgeService])
], JudgeProcessor);
export { JudgeProcessor };
//# sourceMappingURL=judge.processor.js.map