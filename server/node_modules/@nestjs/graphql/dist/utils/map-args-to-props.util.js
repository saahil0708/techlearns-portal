...
const hasRenamedPropsCache = new WeakMap();
const mappedPropsCache = new WeakMap();
function getProps(classRef) {
    ...
    while (!isUndefined(target?.prototype)) {
        const classMetadata = classesMetadata.find((item) => item.target === target);
...
function hasRenamedProps(classRef) {
...
function getMappedProps(classRef) {
    ...
    getProps(classRef).forEach(({ name, schemaName, typeFn }) => {
        const typeRef = typeFn?.();
        const nestedTypeRef = typeof typeRef === 'function' && hasRenamedProps(typeRef)
            ? typeRef
            : undefined;
 ...
 */
export function mapArgsToProps(value, classRef) {
    ...
    const renamed = new Set(mappedProps
...
function resolveArgClass({ index, typeFn }, target, methodName) {
 ...
 */
export function createArgsMapper(target, methodName) {
    const argsTypes = Reflect.getMetadata(ARGS_TYPE_METADATA, target, methodName);
    if (!argsTypes?.length) {
        return null;
    }
    let entries;
    return (args) => {
        if (!entries) {
            entries = argsTypes
                ...
                .filter(({ classRef }) => classRef && hasRenamedProps(classRef));
