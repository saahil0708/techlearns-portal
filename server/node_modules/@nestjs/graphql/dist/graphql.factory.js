...
import { GraphQLObjectType, GraphQLSchema, lexicographicSortSchema, printSchema, } from 'graphql';
...
let GraphQLFactory = class GraphQLFactory {
    ...
    async generateSchema(options = { typeDefs: [] }) {
        ...
        const rawResolvers = extend(this.scalarsExplorerService.explore(), resolvers);
    ...
    overrideOrExtendResolvers(executableSchemaConfig, autoGeneratedSchemaConfig) {
        ...
        rootResolverKeys
            ...
            Object.entries(executableSchemaFields).forEach(([resolverName, value]) => {
        ...
        executableSchemaConfig.types
            ...
            if (implementedInterfaces.length > 0) {
                implementedInterfaces.forEach((interfaceRef) => {
                    ...
                    Object.entries(interfaceRef.getFields()).forEach(([key]) => {
    ...
    async generateDefinitions(typeDefs, options) {
        ...
        const tsFile = await this.graphqlAstExplorer.explore(gql `
        ${typeDefs}
      `, options.definitions.path, options.definitions.outputAs, definitionsGeneratorOptions);
        if (!existsSync(options.definitions.path) ||
