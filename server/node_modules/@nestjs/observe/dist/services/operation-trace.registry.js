...
import { collectCodeFrames } from "../utils/source-context.util.js";
...
export class OperationTraceRegistry {
    ...
    callerIdsByTraceId = new Map();
    logger = new Logger(OperationTraceRegistry.name);
    /**
     ...
     */
    hasReportedMissingInstrumentation = false;
    constructor(als, sourceContext = true) {
        this.als = als;
    ...
    startTrace(traceId, data) {
        if (this.traceSnapshots.has(traceId)) {
            this.logger.warn(`Trace already started for traceId: "${traceId}". Overwriting existing trace.`);
    ...
    endTrace(traceId, opts = {}) {
        ...
        const reportedStatusCode = "statusCode" in opts && typeof opts.statusCode === "number"
            ? opts.statusCode
            : undefined;
        const errorStatusCode = "protocol" in snapshot ? snapshot.errorStatusCode : undefined;
        ...
        const statusCode = errorStatusCode !== undefined &&
     ...
     */
    reportMissingInstrumentation(snapshot) {
        ...
        if (this.hasReportedMissingInstrumentation) {
            this.logger.error(`Operation "${operation}" produced no spans - the "instrument" option is still missing from NestFactory (traceId: "${snapshot.traceId}").`);
            return;
        }
    ...
    setStatusCode(snapshot, statusCode) {
        const requestSnapshot = snapshot;
    ...
    internalStartTraceStep(traceId, className, methodKey, callerId) {
        const snapshot = this.traceSnapshots.get(traceId);
        if (!snapshot) {
            return undefined;
        }
     ...
     */
    attachToCaller(snapshot, refsByCaller, callerId, newNode) {
    ...
    callerId, error) {
        const snapshot = this.traceSnapshots.get(traceId);
        if (!snapshot) {
            this.logger.error(`No snapshot found for traceId: "${traceId}". Cannot end trace step for "${className}#${methodKey}".`);
            return;
        }
        ...
        if (!refsByCaller) {
            this.logger.warn(`No ongoing ref map found for the "${spanId}" span. Some of your async operations were not awaited and the outer span ended before the inner one. Check your "${className}#${methodKey}" method. Ignore if this is the desired behavior.`);
            return;
        }
        const cursor = refsByCaller.get(callerId);
        if (!cursor) {
            this.logger.warn(`No ongoing trace found for the "${spanId}" span. Some of your async operations were not awaited and the outer span ended before the inner one. Check your "${className}#${methodKey}" method. Ignore if this is the desired behavior.`);
            return;
        }
    ...
    addRouteMetadataToTrace(requestId, requestMethod, path) {
        const snapshot = this.traceSnapshots.get(requestId);
        if (!snapshot) {
            return;
        }
     ...
     */
    addGraphQLMetadataToTrace(traceId, metadata) {
        const snapshot = this.traceSnapshots.get(traceId);
        if (!snapshot) {
            return;
        }
     ...
     */
    toStartOffset(snapshot, startTime) {
        if (typeof snapshot?.startTimestamp !== "number") {
            return undefined;
        }
        return startTime - snapshot.startTimestamp;
    }
    captureError(traceId, callerId, error, tags) {
        const snapshot = this.traceSnapshots.get(traceId);
        if (!snapshot) {
            this.logger.warn(`No request snapshot found for traceId: "${traceId}". Cannot capture error.`);
            return;
        }
        if (!snapshot.traces || snapshot.traces.length === 0) {
            this.logger.warn(`No traces found for traceId: "${traceId}". Cannot capture error.`);
            return;
        }
        const refsByCaller = this.callerIdsByTraceId.get(traceId);
        const targetSpan = refsByCaller?.get(callerId);
        if (!targetSpan) {
            this.logger.warn(`No active span found for traceId: "${traceId}". Cannot capture error.`);
            return;
        }
        if (!targetSpan.tags) {
            targetSpan.tags = {};
     ...
     */
    abandonTrace(traceId) {
        this.callerIdsByTraceId.delete(traceId);
        this.traceSnapshots.delete(traceId);
    }
    async pluckSnapshot(traceId) {
        const snapshot = this.traceSnapshots.get(traceId);
        if (snapshot) {
            const cleanup = () => {
                this.callerIdsByTraceId.delete(traceId);
            ...
            if (snapshot.refsCounter > snapshot.refsMarkedAsComplete) {
                await new Promise((resolve) => setTimeout(resolve, SNAPSHOT_COMPLETION_TIMEOUT_MS));
                if (snapshot.refsCounter > snapshot.refsMarkedAsComplete) {
                    cleanup();
                    return undefined;
                }
    ...
    getActiveSpan(traceId, callerId) {
        const refsByCaller = this.callerIdsByTraceId.get(traceId);
        if (!refsByCaller) {
            return undefined;
        }
    ...
    async createManualSpan(traceId, callerId, name, spanFunction) {
        ...
        const onResponse = (res) => {
            // setImmediate
            this.internalEndTraceStep(traceId, newNodeId, manualSpan.className, manualSpan.methodKey, newNodeId);
            return res;
        };
        const onError = (err) => {
            // setTimeout(
            //   () =>
            this.internalEndTraceStep(traceId, newNodeId, manualSpan.className, manualSpan.methodKey, newNodeId, err);
            //   0,
            // );
        ...
        return this.als.run(new Map([...(store?.entries() ?? []), [CALLER_METADATA_KEY, newNodeId]]), () => {
            try {
