...
createSanitizer) => {
     ...
     */
    const sanitizer = typeof createSanitizer === "function" && config.wireShapes
     ...
     */
    const rateLimitPauseMs = (response) => {
    ...
    const gzipAsync = (data) => new Promise((resolve, reject) => zlib.gzip(data, (err, compressed) => {
        if (err) {
            reject(err);
        }
        else {
    ...
    const postBatch = async (json) => fetch(telemetryUrl, {
        method: "POST",
        headers,
     ...
     */
    const repairAndResend = async (json) => {
        if (!sanitizer) {
            return false;
        }
        try {
            ...
            if (retry.ok) {
                ...
                parentPort.postMessage(`Error: Telemetry batch was rejected (400) and repaired: ${summary}. The repaired batch was sent.`);
            }
            else {
                const retryBody = await retry.text().catch(() => "");
                parentPort.postMessage(`Error: Telemetry batch was rejected (400), repaired (${summary}), and rejected again ` +
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 401 || response.status === 403) {
                    ...
                    if (authRejections === 1) {
                        parentPort.postMessage(`Error: Telemetry rejected (${response.status}). Check that appKey and appSecret are valid; ` +
                        ...
                        parentPort.postMessage(`Error: Telemetry still rejected (${response.status}); ${authRejections} batches dropped since start-up.`);
                    }
                    return true;
                }
                if (response.status === 429) {
                    ...
                    parentPort.postMessage(`Error: Telemetry rate-limited (429) - the account may be out of credits or over budget. ` +
                ...
                parentPort.postMessage(`Error: Failed to send data. ${response.statusText} (${response.status})` +
                    (errorBody ? ` - ${errorBody.slice(0, 500)}` : ""));
                return true;
            }
            parentPort.postMessage("Tracing and instrumentation data sent successfully");
            return true;
        }
        catch (err) {
            ...
            parentPort.postMessage(`Error: Could not reach the collector at ${telemetryUrl}` +
                (reason ? ` (${reason})` : "") +
