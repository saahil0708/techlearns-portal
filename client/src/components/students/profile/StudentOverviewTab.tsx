...
export default function StudentOverviewTab({
  ...
  useEffect(() => {
    ...
    apiService
      .getTodayPotd()
      .then((res) => {
        if (active && res && res.problem) {
          setPotd({
            title: res.problem.title,
            slug: res.problem.slug,
            bonusPoints: res.bonusPoints || 50,
            isSolved: res.isSolved || false,
            currentStreak: res.userStreak?.currentStreak,
          });
        }
      })
      .catch(() => {});
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
        ...
        {/* ========================================================================= */}
        {/* PROBLEM OF THE DAY (POTD) & DAILY STREAK CARD */}
        {/* ========================================================================= */}
        <Card
          elevation={0}
          sx={{
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' }, flexDirection: { xs: 'column', sm: 'row' }, gap: 2 }}>
            <Box>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2, mb: 0.8 }}>
                <Chip
                  icon={<LocalFireDepartmentRoundedIcon sx={{ fontSize: 16, color: '#F59E0B !important' }} />}
                  label="Daily Practice Challenge"
                  size="small"
