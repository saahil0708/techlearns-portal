...
import { searchUniversitiesLive } from '@/lib/institution-search-service';
...
export default function CreateInstitutionModal({ open, onClose, onSubmit }: CreateInstitutionModalProps) {
  ...
  const [universityOptions, setUniversityOptions] = useState<RecognizedInstitution[]>(POPULAR_INSTITUTIONS);
  ...
  const handleNameInputChange = (val: string) => {
    ...
    if (!val.trim()) {
      setUniversityOptions(POPULAR_INSTITUTIONS);
    ...
    searchDebounceTimer.current = setTimeout(async () => {
      try {
        const liveResults = await searchUniversitiesLive(val);
        if (liveResults && liveResults.length > 0) {
          setUniversityOptions(liveResults);
        } else {
          setUniversityOptions(
            POPULAR_INSTITUTIONS.filter((p) => p.name.toLowerCase().includes(val.toLowerCase()))
          );
        }
      } catch {
        setUniversityOptions(
  ...
  const handleSelectInstitution = (inst: RecognizedInstitution | string | null) => {
    if (!inst) {
    ...
    if (typeof inst === 'string') {
      handleNameChange(inst);
      return;
    }

    // Selected a recognized institution from the directory
    setFormData((prev) => ({
      ...prev,
      name: inst.name,
      code: inst.code,
      domain: inst.domain || prev.domain,
      tier: inst.tier || prev.tier,
    }));
    ...
    if (inst.city) {
      setSelectedCity(inst.city);
    }
    if (inst.pincode) {
      setPincode(inst.pincode);
      setDetectedLocationInfo(`Auto-detected: ${inst.city || ''}, ${inst.state || ''}`);
    }
  };
      ...
      <form onSubmit={handleSubmit}>
        <DialogContent sx={{ px: 3, pt: 1, pb: 2.5, display: 'flex', flexDirection: 'column', gap: 2.25 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.75 }}>
            {/* Institution Full Name Searchable Dropdown */}
            <Autocomplete
              freeSolo
              options={universityOptions}
              loading={isSearchingUni}
              loadingText="Searching universities directory..."
              noOptionsText="No matching university. Type a custom name."
              getOptionLabel={(option) => (typeof option === 'string' ? option : option.name)}
              value={formData.name}
              onInputChange={(_e, val, reason) => {
                if (reason === 'input') {
                  handleNameInputChange(val);
                }
              }}
              onChange={(_e, val) => {
                handleSelectInstitution(val);
              }}
              renderOption={(props, option) => {
                const { key, ...otherProps } = props;
                const inst = typeof option === 'string' ? { name: option, code: '' } : option;
                return (
                  <Box
                    component="li"
                    key={key}
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25 }}>
                      ...
                      <Box>
                        ...
                        {(inst.city || inst.state || (inst as any).country) && (
                          <Typography sx={{ fontSize: '0.74rem', color: '#64748B' }}>
                            {[inst.city, inst.state, (inst as any).country].filter(Boolean).join(', ')}
                          </Typography>
                        )}
                      </Box>
                    </Box>
              ...
              renderInput={(params) => (
                <TextField
                  ...
                  required
                  helperText="Start typing an alphabet or name to search live universities database"
                  sx={inputStyle}
                  slotProps={{
                    ...params.slotProps,
                    input: {
