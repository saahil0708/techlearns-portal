...
import { getRatingTierInfo, getProblemRating } from '@/utils/codechefRating';
    ...
    try {
      ...
      if (Array.isArray(res?.items)) {
        const mapped = res.items.map((sub: any) => ({
          ...
          score: typeof sub.score === 'number' ? sub.score : (sub.verdict === 'ACCEPTED' ? 100 : 0),
    ...
    try {
      ...
      if (submissionResult?.id) {
        ...
        const newSub = {
          ...
          score: typeof subDetails?.score === 'number' ? subDetails.score : (finalVerdict === 'Accepted' ? 100 : 0),
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 0.8, sm: 1.2 }, minWidth: 0, flexShrink: 1, overflow: 'hidden' }}>
          <Tooltip title="Back to Problem Archive" arrow>
            ...
            >
              <FluidArrowLeft size={18} strokeWidth={2.4} />
            </IconButton>
          </Tooltip>

          {/* Problem Code Badge */}
          <Chip
            label={problem.code || problem.slug?.toUpperCase() || `PRB-${problem.id.slice(0, 6).toUpperCase()}`}
            size="small"
            sx={{
          ...
          {/* CodeChef Star Rating Tier */}
          <Box sx={{ display: { xs: 'none', md: 'inline-flex' } }}>
            <StarRatingBadge
              rating={getProblemRating(problem)}
              showStars={true}
              showDivision={true}
              size="small"
            />
          ...
          >
            ...
            <Box sx={{ borderBottom: '1px solid #F1F5F9', bgcolor: '#F8FAFC', px: 2 }}>
              ...
              >
                ...
                <Tab icon={<TaskAltRoundedIcon sx={{ fontSize: 17 }} />} iconPosition="start" label="Subtasks" />
            ...
            {/* TAB 1: Subtasks & Scoring */}
            {activeTab === 1 && (
              <Box sx={{ p: { xs: 2, sm: 3 }, display: 'flex', flexDirection: 'column', gap: 2.5, overflowY: 'auto' }}>
                <Box>
                  <Typography sx={{ fontSize: '1rem', fontWeight: 800, color: '#0F172A', mb: 0.5 }}>
                    Subtasks & Scoring Breakdown
                  </Typography>
                  <Typography sx={{ fontSize: '0.82rem', color: '#64748B' }}>
                    Points are awarded per subtask upon passing all associated private testcases within the time limit.
                  </Typography>
                </Box>
                ...
                <TableContainer sx={{ border: '1px solid #E2E8F0', borderRadius: '8px' }}>
                  <Table size="small">
                    <TableHead sx={{ bgcolor: '#F8FAFC' }}>
                      <TableRow>
                        <TableCell sx={{ fontWeight: 800, fontSize: '0.74rem', color: '#64748B' }}>SUBTASK</TableCell>
                        <TableCell sx={{ fontWeight: 800, fontSize: '0.74rem', color: '#64748B' }}>POINTS</TableCell>
                        <TableCell sx={{ fontWeight: 800, fontSize: '0.74rem', color: '#64748B' }}>TEST CASES</TableCell>
                        <TableCell sx={{ fontWeight: 800, fontSize: '0.74rem', color: '#64748B' }}>STATUS</TableCell>
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {(problem.subtasks || [
                    ...
                    <Table size="small">
                      <TableHead sx={{ bgcolor: '#F8FAFC' }}>
                        <TableRow>
                          ...
                          <TableCell sx={{ fontWeight: 800, fontSize: '0.74rem', color: '#64748B' }}>SCORE</TableCell>
                      ...
                      <TableBody>
                        {submissions.map((sub) => (
                          <TableRow key={sub.id} hover>
                            ...
                            <TableCell>
                              <Chip
                                label={sub.verdict === 'Evaluating' ? '...' : `${sub.score ?? (sub.verdict === 'Accepted' ? 100 : 0)} pts`}
                                size="small"
                                sx={{
                                  fontWeight: 800,
                                  fontSize: '0.72rem',
